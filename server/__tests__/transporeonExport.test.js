...
test("parseTransporeonExport: erkennt '2. Buchung' als Entlade-Spalten", () => {
  const altHeader = [
    "Entladedatum",
    "Transportnr.",
    "Gebucht ab - Time Slot Management",
    "Ankunft - Time Slot Management",
    "Abfahrt - Time Slot Management",
    "Gebucht ab - 2. Buchung - Time Slot Management",
    "Ankunft - 2. Buchung - Time Slot Management",
    "Abfahrt - 2. Buchung - Time Slot Management",
  ];
  ...
  const out = parseTransporeonExport(rows);
  assert.equal(out.length, 1);
  assert.equal(out[0].unloading.window_local, "2026-07-28 10:30");
  assert.equal(out[0].unloading.arrival_local, "2026-07-28 08:15");
  assert.equal(out[0].unloading.departure_local, "2026-07-28 14:44");
});

test("parseTransporeonExport: Fallback ueber zweite Spaltenposition ohne Entlade-Marker", () => {
  ...
  const out = parseTransporeonExport(rows);
  assert.equal(out.length, 1);
  assert.equal(out[0].loading.window_local, "2026-07-28 04:00");
  assert.equal(out[0].unloading.window_local, "2026-07-28 10:30");
});
